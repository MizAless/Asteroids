...
public class LaserGun : DefaultGun
{
    ...
    public LaserGun(Transformable shootPoint, int bullets, int bulletsPerShot = 1) : base(shootPoint)
    {
        if (bulletsPerShot < 0)
    ...
    {
        Bullets -= _bulletsPerShot;
        return new LaserGunBullet(_shootPoint.Position, _shootPoint.Forward);
    }
}
